#EMPLEADOS QUE SI CHECARON COMEDOR
...
ENCABEZADO   
   ...
   SI_NO
     IMP ( COL(1), 'CLAVE', COL(10), 'NOMBRE', COL(50), 'HORA', COL(60), 'CENTRO DE COSTO' ) IMPRIME
   FIN_SI
   IMP ( REPITETXT ('-', COLUMNAS )) IMPRIME
   LETRA ( 'LETRA NORMAL' )
   IMPRIME
...
SUB_RUTINA formato_trabajador
   ...
   SI ( $salida = 'EXCEL' )
     ...
     EXCEL( 'PON_VALOR', 3, _ren_excel, HORA( 'EMPCOME':'HORA' ) )
     EXCEL( 'PON_VALOR', 4, _ren_excel, $depto )
...
SI ( $salida = 'EXCEL' )
  ...
  EXCEL( 'PON_VALOR', 3, _ren_excel, 'HORA' )
  EXCEL( 'PON_VALOR', 4, _ren_excel, 'DEPARTAMENTO' )
  _ren_excel := _ren_excel + 1
FIN_SI
...
MIENTRAS ( FIN_BASE ( 'EMPPRIN' ) = FALSO )
  SI ( ES_VIGENTE( _fecha) )
    SI ( TRAE_REGISTRO( 'EMPCOME', 'EMPPRIN':'CLAVE', _fecha ) = VERDADERO )
      datos_trabajador
    FIN_SI
  FIN_SI
